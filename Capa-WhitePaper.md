...
The Capa distribution exposes a single `capa` command (also reachable as `python -m capa`) with several modes. As of v0.5.0-alpha:

- **Pipeline modes**: tokenise (no flag), `--parse` (print the AST), `--check` (full semantic analysis, including the capability discipline), `--transpile` (emit equivalent Python), `--run` (execute).
- **`capa --fmt` / `--fmt-check`**: the canonical code formatter, single and non-configurable in the spirit of `gofmt`. v1 is line-level: line-ending normalisation, indentation snapped to four-space multiples, blank-line clusters collapsed, trailing whitespace stripped, final newline enforced, block-comment interiors preserved verbatim. v2 (intra-line operator spacing via an AST round-trip with comment preservation) is queued.
- **`capa --doc`**: the documentation generator. Reads `///` and `/** */` doc comments attached to declarations and emits a self-contained HTML page with paragraphs, inline `` `code` ``, fenced code blocks (with optional language tag), bulleted lists, a separate Traits section, and capability badges.
- **`capa --manifest` / `--cyclonedx`**: capability manifest as JSON, or a CycloneDX 1.5 SBOM with the manifest embedded.
- **`capa init [name]`**: scaffolds a runnable starter project (`main.capa` using `Stdio`, `README.md`, `.gitignore`, `.capa-version`).
- **`capa lsp`**: the language server (an optional `[lsp]` extra brings in `pygls>=2.0`). The v1 server delivers diagnostics, hover, go-to-definition, find-references, document symbols (hierarchical outline), and code actions (Quick Fix for "did you mean?" hints). Both reference and declaration sites are covered. Editor configuration is one-line for Helix, Neovim, Zed, and VSCode (via LSP-client extensions); a richer first-party VSCode extension that ships the LSP client out of the box is queued.
...
Status as of v0.5.0-alpha: the formatter (`capa --fmt` v1, line-level), the documentation generator (`capa --doc`, with paragraphs, inline code, fenced code blocks, lists, and a Traits section), the project scaffolder (`capa init`), and the language server (`capa lsp` v1, with diagnostics, hover, go-to-definition, find-references, document symbols, code actions) have shipped. Standard-library work (the stdlib surface beyond the built-in capabilities) and richer first-party editor extensions are the remaining items in this phase.
