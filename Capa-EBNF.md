...
The `range_expr` production allows integer ranges. `a..b` is exclusive of `b`, `a..=b` is inclusive. Range itself is non-associative, `a..b..c` is a syntax error, and sits below comparisons in precedence, so `0..n == 0..m` parses as `(0..n) == (0..m)`. The value of a range expression has type `Range<Int>`, a distinct type from `List<Int>`: it is not indexable and does not coerce to a list, so `to_list()` is the way across. `Range<T>` carries twelve methods, a subset of the `List<T>` API (see [`docs/stdlib.md`](docs/stdlib.md)). Endpoints must be `Int`; Float endpoints are rejected at type-check time.
