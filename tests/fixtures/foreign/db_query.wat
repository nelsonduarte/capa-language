(module
  (import "capa:host/db" "query"
    (func $query (param i32 i32 i32 i32 i32 i32)))
  ...
  ;; Call db.query(handle, path, sql) and marshal result<string, io-error>
  ;; to a plain returned string -- the Ok JSON on success, the literal
  ;; "ERR" marker on the Err arm. The handle is ignored by the host
  ;; closure (it uses the captured attenuated cap).
  (func $run_query (export "run-query")
      (param $pptr i32) (param $plen i32) (param $qptr i32) (param $qlen i32)
      (result i32)
    ...
    (call $query (i32.const 0)
      (local.get $pptr) (local.get $plen)
      (local.get $qptr) (local.get $qlen)
      (local.get $area))
