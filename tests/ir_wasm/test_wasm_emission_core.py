...
# ``python -m unittest``.
"""WebAssembly backend: emission core (WAT shape, assembly, the type-
token sanitiser, and Int / Bool execution).

This is the emission-core module of the tests/ir_wasm package; see
tests/ir_wasm/__init__.py for the growth convention and the three
pipeline levels (WAT shape -> wasm-tools parse -> wasmtime execution)
these tests exercise. The shared _parse_lower / skip gates live in
