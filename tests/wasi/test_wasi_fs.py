"""WASI mode: the Fs capability (wasi:filesystem).

Preopen flag guards, static fs-ceiling analysis, WIT generation + rejections,
the dynamic-preopen compile surface, preopen-spec parsing, WIT license headers,
and the end-to-end exists / mkdir / read / write / write-only / list-dir /
dynamic-preopen / attenuation runs. Split out of tests/test_wasi_mode.py; see
tests/wasi/__init__.py for the growth convention.

This module is intentionally kept whole for now even though it is large; when
the dynamic-preopen facet warrants it, split along that named seam into a future
test_wasi_fs_dynamic.py rather than chunking by line count. The shared
primitives live in tests/wasi/_helpers.py.
"""
