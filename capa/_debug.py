...
summary that names the originating ``file:line`` for each transpiled
frame.
...
map key ``<= frame.lineno`` (the statement the line belongs to).
"""
...
def _rewrite_traceback(exc_info, line_map: dict) -> str:
    ...
    rendered as ``file:line``. Returns the empty string when no frame
    maps, so the caller falls back to the plain Python traceback."""
    ...
    for frame in frames:
        ...
        rendered.append(f"  {where}:{pos.line}")
