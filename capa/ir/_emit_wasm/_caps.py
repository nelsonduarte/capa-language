...
class _CapDispatchMixin:
    ...
    def _emit_wasi_fs_write_call(self, instr: MethodCall) -> None:
        """Emit ``fs.write(path, content)`` in WASI mode.

        The PATH argument (arg[0]) MUST be a string literal (the fail-
        closed ceiling guarantees this; ``_validate_wasi_caps`` rejects a
        dynamic path before emission). The CONTENT argument (arg[1]) is
        any String (literal / local / param) whose bytes already live in
        linear memory; it is pushed as ``(ptr, len)`` via the standard
        ``_push_string_arg`` and handed straight to the wrapper (no copy).

        Resolve the literal path to ``(preopen_index, relative_path)``
        via the static Fs ceiling, intern the relative path, allocate the
        20-byte canonical-ABI ret area, call the ``$Fs_write`` guest
        wrapper with ``(idx, rel_ptr, rel_len, content_ptr, content_len,
        ret_area)``, and reuse the shared ``result_unit_io_error``
        materialiser so the lifted value is a ``Result<Unit, IoError>``
        identical to the capa:host write.

        ``$Fs_write`` itself (in ``_wasi.py``) does the open-at (create|
        truncate, write) -> write-via-stream -> blocking-write-and-flush
        loop -> blocking-flush over the host preopens, with all resource
        drops; this call site only resolves the path, pushes the content,
        and lifts the result."""
        from .._fs_ceiling import resolve_fs_call
        if len(instr.args) != 2:
            raise WasmEmissionError(
        ...
        self._write(f"i32.const {rel_off}")
        self._write(f"i32.const {rel_len}")
        # content (ptr, len) - the bytes already live in linear memory.
