...
Owns the self-contained Wasm helper functions every emitted module
ships with:
...
- ``$ftoa(f: f64) -> (i32 ptr, i32 len)`` -- shortest-round-trip
  decimal formatting for ``${float}`` interpolation, matching
  Python's ``str(float)`` for the value ranges Capa cares about.
  Decomposes into ``$grisu_mul_high``, ``$grisu_cached_power``,
  ``$grisu2``, and ``$pow10_i32``; the cached-powers table sits
  at the offset reserved by the dispatcher before string emission.

These are pure emission methods: they only call ``self._write`` and
read/bump ``self._block_counter`` / ``self._indent``. Splitting them
into a mixin removes ~400 lines from ``__init__.py`` without
