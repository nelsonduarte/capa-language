...
class Analyzer:
    ...
    def _install_builtins(self) -> None:
        ...
        # type-name resolver can find them. There is no associated value.
        for name in PRIMITIVE_NAMES:
            self.global_scope.define(
                Symbol(name=name, kind=SymbolKind.TYPE_STRUCT, pos=_BUILTIN_POS)
            )
        # Recognized capabilities. They are opaque types from the v1 checker's point of view.
        for name in CAPABILITY_NAMES:
            self.global_scope.define(
                Symbol(name=name, kind=SymbolKind.CAPABILITY, pos=_BUILTIN_POS)
            )
        # Pre-defined generic types with their type params.
        for name, params in (
            ("List", ["T"]),
            ("Option", ["T"]),
            ("Result", ["T", "E"]),
            ...
            self.global_scope.define(
                Symbol(
                    ...
                    pos=_BUILTIN_POS, type_params=params,
                )
            )
        # Builtin methods of List<T>. T is the receiver's type param;
        # U (in map) and Bool (in filter) are fixed by the shape.
        list_sym = self.global_scope.lookup("List")
        if list_sym is not None:
            T = TyVar("T")
            U = TyVar("U")
            # length() -> Int
            list_sym.methods["length"] = Symbol(
            ...
            # contains(x: T) -> Bool
            list_sym.methods["contains"] = Symbol(
                name="contains", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((T,), TyBool),
            )
            # map<U>(f: Fun(T) -> U) -> List<U>
            list_sym.methods["map"] = Symbol(
                name="map", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun(
                    (TyFun((T,), U),),
                    TyName("List", (U,)),
                ),
            ...
            # filter(p: Fun(T) -> Bool) -> List<T>
            list_sym.methods["filter"] = Symbol(
                name="filter", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun(
                    (TyFun((T,), TyBool),),
                    TyName("List", (T,)),
                ),
            )
            # fold<U>(init: U, f: Fun(U, T) -> U) -> U
            list_sym.methods["fold"] = Symbol(
                name="fold", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun(
                    (U, TyFun((U, T), U)),
                    U,
                ),
            ...
            # is_empty() -> Bool
            list_sym.methods["is_empty"] = Symbol(
                name="is_empty", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((), TyBool),
            )
            # first() -> Option<T>, last() -> Option<T>
            list_sym.methods["first"] = Symbol(
                name="first", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((), TyName("Option", (T,))),
            )
            list_sym.methods["last"] = Symbol(
                name="last", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((), TyName("Option", (T,))),
            )
            # get(i: Int) -> Option<T>  (safe access by index)
            list_sym.methods["get"] = Symbol(
                name="get", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((TyInt,), TyName("Option", (T,))),
            )
        ...
        # length() -> Int
        string_sym.methods["length"] = Symbol(
            name="length", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
            ty=TyFun((), TyInt),
        )
        # contains(s: String) -> Bool
        string_sym.methods["contains"] = Symbol(
            name="contains", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
            ty=TyFun((TyString,), TyBool),
        )
        # starts_with(s: String) -> Bool
        string_sym.methods["starts_with"] = Symbol(
            name="starts_with", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
            ty=TyFun((TyString,), TyBool),
        )
        # ends_with(s: String) -> Bool
        string_sym.methods["ends_with"] = Symbol(
            name="ends_with", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
            ty=TyFun((TyString,), TyBool),
        )
        # to_upper() -> String
        string_sym.methods["to_upper"] = Symbol(
            name="to_upper", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
            ty=TyFun((), TyString),
        )
        # to_lower() -> String
        string_sym.methods["to_lower"] = Symbol(
            name="to_lower", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
            ty=TyFun((), TyString),
        )
        # trim() -> String
        string_sym.methods["trim"] = Symbol(
            name="trim", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
            ty=TyFun((), TyString),
        )
        # split(sep: String) -> List<String>
        string_sym.methods["split"] = Symbol(
            name="split", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
            ty=TyFun((TyString,), TyName("List", (TyString,))),
        )
        # replace(old: String, new_: String) -> String
        string_sym.methods["replace"] = Symbol(
            name="replace", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
            ty=TyFun((TyString, TyString), TyString),
        )
        # is_empty() -> Bool
        string_sym.methods["is_empty"] = Symbol(
            name="is_empty", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
            ty=TyFun((), TyBool),
        )
        self.global_scope.define(string_sym)

        # Builtin methods of Map<K, V>. K and V are the type's type params.
        map_sym = self.global_scope.lookup("Map")
        if map_sym is not None:
            K = TyVar("K")
            V = TyVar("V")
            # length() -> Int
            map_sym.methods["length"] = Symbol(
                name="length", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((), TyInt),
            )
            # get(k: K) -> Option<V>
            map_sym.methods["get"] = Symbol(
                name="get", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((K,), TyName("Option", (V,))),
            )
            # set(k: K, v: V) -> ()  (mutating)
            map_sym.methods["set"] = Symbol(
                name="set", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((K, V), TyUnit),
            )
            # contains_key(k: K) -> Bool
            map_sym.methods["contains_key"] = Symbol(
                name="contains_key", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((K,), TyBool),
            )
            # keys() -> List<K>
            map_sym.methods["keys"] = Symbol(
                name="keys", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((), TyName("List", (K,))),
            )
            # values() -> List<V>
            map_sym.methods["values"] = Symbol(
                name="values", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((), TyName("List", (V,))),
            )
            # is_empty() -> Bool
            map_sym.methods["is_empty"] = Symbol(
                name="is_empty", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((), TyBool),
            )

        # Builtin methods of Set<T>.
        set_sym = self.global_scope.lookup("Set")
        if set_sym is not None:
            T = TyVar("T")
            set_sym.methods["length"] = Symbol(
                name="length", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((), TyInt),
            )
            set_sym.methods["add"] = Symbol(
                name="add", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((T,), TyUnit),
            )
            set_sym.methods["remove"] = Symbol(
                name="remove", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((T,), TyUnit),
            )
            set_sym.methods["contains"] = Symbol(
                name="contains", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((T,), TyBool),
            )
            set_sym.methods["to_list"] = Symbol(
                name="to_list", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((), TyName("List", (T,))),
            )
            set_sym.methods["is_empty"] = Symbol(
                name="is_empty", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((), TyBool),
            )

        # Builtin functions to create empty Map and Set. Without
        # explicit type args, they return Map<?, ?> / Set<?>; the user
        # annotates the let to fix the types: ``let m: Map<String, Int> = new_map()``.
        self.global_scope.define(
            Symbol(
                name="new_map", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((), TyName("Map", (TyUnknown, TyUnknown))),
            )
        )
        self.global_scope.define(
            Symbol(
                name="new_set", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((), TyName("Set", (TyUnknown,))),
            )
        )

        # Methods of the Stdio capability. Capabilities have special
        # handling in method dispatch, but if ``methods`` is populated,
        # it is consulted first to type precisely.
        stdio_sym = self.global_scope.lookup("Stdio")
        if stdio_sym is not None:
            stdio_sym.methods["print"] = Symbol(
                name="print", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((TyString,), TyUnit),
            )
            stdio_sym.methods["println"] = Symbol(
                name="println", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((TyString,), TyUnit),
            )
            stdio_sym.methods["eprintln"] = Symbol(
                name="eprintln", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((TyString,), TyUnit),
            )
            stdio_sym.methods["read_line"] = Symbol(
                name="read_line", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((), TyName("Result", (TyString, TyName("IoError")))),
            )

        # Methods on the Fs capability.
        fs_sym = self.global_scope.lookup("Fs")
        if fs_sym is not None:
            io_err = TyName("IoError")
            res_str = TyName("Result", (TyString, io_err))
            res_unit = TyName("Result", (TyUnit, io_err))
            fs_sym.methods["read"] = Symbol(
                name="read", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((TyString,), res_str),
            )
            fs_sym.methods["write"] = Symbol(
                name="write", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((TyString, TyString), res_unit),
            )
            fs_sym.methods["exists"] = Symbol(
                name="exists", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((TyString,), TyBool),
            )

        # Methods of the Env capability.
        env_sym = self.global_scope.lookup("Env")
        if env_sym is not None:
            env_sym.methods["get"] = Symbol(
                name="get", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((TyString,), TyName("Option", (TyString,))),
            )
            env_sym.methods["args"] = Symbol(
                name="args", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((), TyName("List", (TyString,))),
            )

        # Methods of the Clock capability.
        clock_sym = self.global_scope.lookup("Clock")
        if clock_sym is not None:
            clock_sym.methods["now_secs"] = Symbol(
                name="now_secs", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((), TyFloat),
            )
            clock_sym.methods["now_monotonic"] = Symbol(
                name="now_monotonic", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((), TyFloat),
            )
            clock_sym.methods["sleep"] = Symbol(
                name="sleep", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
        ...
        if random_sym is not None:
            random_sym.methods["int_range"] = Symbol(
                name="int_range", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((TyInt, TyInt), TyInt),
            )
            random_sym.methods["float_unit"] = Symbol(
                name="float_unit", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((), TyFloat),
            )

        # Builtin conversion functions. They return Option<T> so the
        # caller explicitly handles the invalid-input case.
        self.global_scope.define(
            Symbol(
                name="parse_int", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((TyString,), TyName("Option", (TyInt,))),
            )
        )
        self.global_scope.define(
            Symbol(
                name="parse_float", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((TyString,), TyName("Option", (TyFloat,))),
            )
        )
        ...
        # type reflects that.
        unsafe_ty = TyName("Unsafe")
        self.global_scope.define(
            Symbol(
                name="py_import", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((unsafe_ty, TyString), TyUnknown),
            )
        )
        self.global_scope.define(
            Symbol(
                name="py_invoke", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((unsafe_ty, TyUnknown, TyName("List", (TyUnknown,))), TyUnknown),
            )
        )

        # JsonValue: built-in sum type for JSON representation.
        # Variants: JNull, JBool(Bool), JNum(Float), JStr(String),
        # JArr(List<JsonValue>), JObj(Map<String, JsonValue>).
        json_sym = Symbol(
            name="JsonValue", kind=SymbolKind.TYPE_SUM, pos=_BUILTIN_POS,
            type_params=[],
        )
        self.global_scope.define(json_sym)
        json_ty = TyName("JsonValue")

        def _add_jvariant(name: str, payload_ty: Optional[Ty]) -> None:
            vsym = Symbol(
                name=name, kind=SymbolKind.VARIANT, pos=_BUILTIN_POS,
                variant_owner=json_sym, variant_payload_ty=payload_ty,
            )
            self.global_scope.define(vsym)
            json_sym.sum_variants[name] = vsym

        _add_jvariant("JNull", None)
        _add_jvariant("JBool", TyBool)
        _add_jvariant("JNum", TyFloat)
        _add_jvariant("JStr", TyString)
        _add_jvariant("JArr", TyName("List", (json_ty,)))
        _add_jvariant("JObj", TyName("Map", (TyString, json_ty)))

        # Extraction methods: each returns Option<T> or Bool. They allow
        # ergonomic access without match boilerplate at every call.
        json_sym.methods["is_null"] = Symbol(
            name="is_null", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
            ty=TyFun((), TyBool),
        )
        json_sym.methods["as_bool"] = Symbol(
            name="as_bool", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
            ty=TyFun((), TyName("Option", (TyBool,))),
        )
        json_sym.methods["as_num"] = Symbol(
            name="as_num", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
            ty=TyFun((), TyName("Option", (TyFloat,))),
        )
        json_sym.methods["as_string"] = Symbol(
            name="as_string", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
            ty=TyFun((), TyName("Option", (TyString,))),
        )
        json_sym.methods["as_array"] = Symbol(
            name="as_array", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
            ty=TyFun((), TyName("Option", (TyName("List", (json_ty,)),))),
        )
        json_sym.methods["as_object"] = Symbol(
            name="as_object", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
            ty=TyFun((),
                TyName("Option", (TyName("Map", (TyString, json_ty)),))
            ),
        )

        # Builtin functions for JSON.
        self.global_scope.define(
            Symbol(
                name="parse_json", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((TyString,), TyName("Result", (json_ty, TyString))),
            )
        )
        self.global_scope.define(
            Symbol(
                name="to_json", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((json_ty,), TyString),
            )
        )
        # Option and Result variants, registered as constructors in the global scope.
        # They use a TyVar corresponding to the owner's type_param to enable
        # inference: Some(42) -> mapping{T:Int} -> Option<Int>.
        opt_sym = self.global_scope.lookup("Option")
        self.global_scope.define(
            Symbol(
                name="Some", kind=SymbolKind.VARIANT, pos=_BUILTIN_POS,
                variant_owner=opt_sym, variant_payload_ty=TyVar("T"),
            )
        )
        self.global_scope.define(
            Symbol(
                name="None", kind=SymbolKind.VARIANT, pos=_BUILTIN_POS,
                variant_owner=opt_sym,
            )
        )
        # Methods of Option<T>: is_some, is_none, unwrap_or, map,
        # and_then, ok_or.
        if opt_sym is not None:
            T = TyVar("T")
            U = TyVar("U")
            E = TyVar("E")
            opt_sym.methods["is_some"] = Symbol(
                name="is_some", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((), TyBool),
            )
            opt_sym.methods["is_none"] = Symbol(
                name="is_none", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((), TyBool),
            )
            opt_sym.methods["unwrap_or"] = Symbol(
                name="unwrap_or", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((T,), T),
            )
            # map<U>(f: Fun(T) -> U) -> Option<U>
            opt_sym.methods["map"] = Symbol(
                name="map", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun(
                    (TyFun((T,), U),),
                    TyName("Option", (U,)),
            ...
            opt_sym.methods["and_then"] = Symbol(
                name="and_then", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun(
                    (TyFun((T,), TyName("Option", (U,))),),
                    TyName("Option", (U,)),
                ),
                type_params=["U"],
            )
            # ok_or<E>(err: E) -> Result<T, E>
            opt_sym.methods["ok_or"] = Symbol(
                name="ok_or", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((E,), TyName("Result", (T, E))),
                type_params=["E"],
            )
        res_sym = self.global_scope.lookup("Result")
        self.global_scope.define(
            Symbol(
                name="Ok", kind=SymbolKind.VARIANT, pos=_BUILTIN_POS,
                variant_owner=res_sym, variant_payload_ty=TyVar("T"),
            )
        )
        self.global_scope.define(
            Symbol(
                name="Err", kind=SymbolKind.VARIANT, pos=_BUILTIN_POS,
                variant_owner=res_sym, variant_payload_ty=TyVar("E"),
            )
        )
        # Methods of Result<T, E>: is_ok, is_err, unwrap_or, map,
        # and_then, map_err.
        if res_sym is not None:
            ...
            E = TyVar("E")
            F = TyVar("F")
            res_sym.methods["is_ok"] = Symbol(
                name="is_ok", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((), TyBool),
            )
            res_sym.methods["is_err"] = Symbol(
                name="is_err", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((), TyBool),
            )
            res_sym.methods["unwrap_or"] = Symbol(
                name="unwrap_or", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun((T,), T),
            )
            # map<U>(f: Fun(T) -> U) -> Result<U, E>
            res_sym.methods["map"] = Symbol(
                name="map", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun(
                    (TyFun((T,), U),),
                    TyName("Result", (U, E)),
            ...
            res_sym.methods["and_then"] = Symbol(
                name="and_then", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun(
                    (TyFun((T,), TyName("Result", (U, E))),),
                    TyName("Result", (U, E)),
                ),
                type_params=["U"],
            )
            # map_err<F>(f: Fun(E) -> F) -> Result<T, F>
            res_sym.methods["map_err"] = Symbol(
                name="map_err", kind=SymbolKind.FUNCTION, pos=_BUILTIN_POS,
                ty=TyFun(
                    (TyFun((E,), F),),
                    TyName("Result", (T, F)),
                ),
